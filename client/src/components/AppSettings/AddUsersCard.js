...
const AddUsers = ({ app, setApp }) => {
  ...
  const appOwnerLogin =
    users.length !== 0
      ? users.find((user) => user.userId === app.userId).githubLogin
      : "";
  useEffect(() => {
  ...
  const handleFormSubmit = async (updatedUsersLogins) => {
    const updatedUsersIds = updatedUsersLogins.map(
      (userLogin) => users.find((user) => user.githubLogin === userLogin).userId
    );
